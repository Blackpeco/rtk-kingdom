...
9. Forest wanderer must **not** start Battle while `MenuOpen` (party panel **or** ยายเมือง dialog). They still roam. Close the UI; if you are still overlapping, Stay starts the fight (same save-on-enter / grace lock). Standing still when the 1.6s return grace ends does **not** auto-start — walk into a pack, or close P/E while overlapping.
