...
Open `Assets/Scenes/Battle.unity` and press **Play**. Direct Play uses the wired 3v3 below — it does **not** load a CharacterCreate save, does **not** write `ts_online_save.json` when the fight ends, and does **not** set `LastEnd` (so Play `World.unity` afterward is not treated as a return). World encounters still use the live party and persist as usual.
