...
**F9** mid-World reloads the file onto the current session (position included). It also clears leftover encounter Stay-retry flags **and** ยายเมือง `_near` / `_talking` (a `transform` teleport can skip Exit2D). Open **P** while a pack is overlapping, **F9** to a city save, close **P** — must **not** start Battle. Walk into a pack with menus closed still does. Talk near grandma, **F9** to a forest save — no **กด E** prompt and no stuck dialog; **F9** onto grandma and **E** works again.
...
2. **P** → add generals until 5 → close. (A wanderer touching you while P is open must **not** start Battle; closing P while still overlapping does start it. After a return, standing still when grace ends must **not** auto-start.)
