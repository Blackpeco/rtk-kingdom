...
After a Battle-only win, **ดำเนินการต่อ** allocates points **in memory only** — it does **not** write `ts_online_save.json` and does **not** set `LastEnd`. That keeps the test 3v3 from overwriting a created-hero save or faking a World return (wrong spawn) if you then Play `World.unity` with Domain Reload off. World encounters still save on battle end and again after **กลับโลก**.
